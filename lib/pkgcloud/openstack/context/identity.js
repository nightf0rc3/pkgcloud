...
Identity.prototype._buildAuthenticationPayload = function () {
  ...
  if (self.options.keystoneAuthVersion === 'v3') {
    ...
    // application credentials are also valid inputs
    else if (self.options.credentialId && self.options.credentialSecret) {
      self._authenticationPayload = {
        auth: {
          identity : {
            methods : ['application_credential'],
            application_credential: {
              id: self.options.credentialId,
              secret: self.options.credentialSecret
            }
          }
        }
      };
